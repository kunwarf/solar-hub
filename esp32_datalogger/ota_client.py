...
def http_get_json(url, timeout=10):
    """
    Simple HTTP GET with JSON response (MicroPython compatible).
    ...
    try:
        ...
        if url.startswith("http://"):
            ...
            raise ValueError("HTTPS not supported")
...
class OTAClient:
    ...
    def __init__(self, config):
        ...
        self.check_interval = config.get("ota", {}).get("check_interval", 300)  # 5 minutes default
        self.last_check = 0
        self.updating = False
    ...
    def check_for_update(self):
        ...
        # Get device info (memory, uptime, etc.)
        device_info = {
        ...
        try:
            ...
            for i, file_info in enumerate(files):
                ...
                if actual_checksum != checksum:
                    ...
                    if is_required:
                        self._report_status("failed", 0, "Checksum mismatch: {}".format(filename))
                        return False
                    else:
                        continue
                ...
                if not FileManager.save_file(filename, content):
                    ...
                    if is_required:
                        self._report_status("failed", 0, "Failed to save: {}".format(filename))
                        return False

                # Report progress
                progress = int(((i + 1) / total_files) * 90)  # 90% for download
