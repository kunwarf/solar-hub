"""
Serial port wrapper for ESP8266 Data Logger.
...
----------------------
- UART0 (GPIO1=TX, GPIO3=RX) is the only full-duplex UART.
  machine.UART() on ESP8266 does NOT accept tx=/rx= keyword arguments;
  pin assignment is fixed by the hardware.
- UART1 (GPIO2=TX) is TX-only — cannot receive, do not use for RS485.
- The rxbuf= parameter is not supported on ESP8266; omit it.
- The static _RX_BUF is reduced to 2 KB (vs 12 KB on ESP32) because
  the ESP8266 only has ~80 KB of usable heap.  If you need to receive
  larger frames (e.g., multiple JK BMS units), reduce polling frequency
  to give the heap time to recover between allocations.
"""
import machine
import time
...
# Static receive buffer — 2 KB (reduced from 12 KB on ESP32).
# Using a fixed buffer and uart.readinto() eliminates per-poll heap
# allocation in read_available().  On ESP8266 this matters even more
# than on ESP32 because heap is much more limited.
_RX_BUF = bytearray(2048)
...
class SerialPort:
    ...
    def __init__(self, config):
        ...
        Args:
            config: Serial configuration dict with uart_id, baudrate,
                    parity, stop_bits, data_bits, response_timeout_ms,
                    prompt, line_ending.  Optional: de_pin (RS485 direction
                    control), frame_header (binary protocol header bytes).
                    Note: tx_pin and rx_pin are ignored on ESP8266.
        """
        self.config = config
        self.uart = None
    ...
    def _init_uart(self):
        ...
        ESP8266 difference: tx=/rx= keyword arguments and rxbuf= are not
        supported.  Pins are fixed: UART0 → GPIO1 (TX), GPIO3 (RX).
        """
        cfg = self.config
        ...
        rs485_info = ""
        if (de_pin_num is not None and de_pin_num >= 0) or \
                (re_pin_num is not None and re_pin_num >= 0):
            rs485_info = " RS485-DE={} RE={}".format(
                de_pin_num if de_pin_num is not None and de_pin_num >= 0 else "off",
                re_pin_num if re_pin_num is not None and re_pin_num >= 0 else "off"
            )
        print("[Serial] UART{} initialized: {}baud{}".format(
            cfg["uart_id"], cfg.get("baudrate", 115200), rs485_info
        ))
    ...
    def read_frame(self, header=None, max_len=512, timeout_ms=None):
        ...
        if not header_found or len(buf) <= len(header):
            print("[Serial] read_frame: timeout after {}ms, {} bytes rx, "
                  "header_found={}{}".format(
                      timeout_ms, len(buf), header_found,
                      (" first={}".format(
                          " ".join("{:02X}".format(b) for b in buf[:8])) if buf else ""),
                  ))
            return None
        return buf
    ...
    def read_available(self, idle_ms=500, timeout_ms=None):
        ...
        Uses the module-level static _RX_BUF (2 KB on ESP8266) via
        uart.readinto() to avoid heap allocation during the read.

        Args:
            idle_ms:    Bus-idle threshold in milliseconds (default 500 ms).
            timeout_ms: Maximum total wait in milliseconds.

        Returns:
            memoryview of received bytes, or None if no data arrived.
        """
        if timeout_ms is None:
            timeout_ms = self.config.get("response_timeout_ms", 5000)
